...
static int disc_chr_cb(uint16_t conn_handle, const struct ble_gatt_error *error, const struct ble_gatt_chr *chr,
  ...
  if (uuid_matches_(&chr->uuid, &write_uuid)) {
    ...
    ESP_LOGI(TAG, "Found write characteristic handle=%d", chr->val_handle);
  }
  return 0;
}
...
static int disc_svc_cb(uint16_t conn_handle, const struct ble_gatt_error *error, const struct ble_gatt_svc *svc,
  ...
  ESP_LOGI(TAG, "Found service, discovering characteristics");
  return ble_gattc_disc_all_chrs(conn_handle, svc->start_handle, svc->end_handle, disc_chr_cb, arg);
}
  ...
  switch (event->type) {
    ...
    case BLE_GAP_EVENT_CONNECT:
      if (event->connect.status == 0) {
        ...
        ble_gattc_disc_all_svcs(self->conn_handle_, disc_svc_cb, self);
      } else {
